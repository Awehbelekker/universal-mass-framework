...
if (-not $DryRun) {
    $systemdContent | Out-File -FilePath "config/mass-security-monitoring.service" -Encoding utf8
...
if (-not $DryRun) {
    $windowsServiceContent | Out-File -FilePath "config/windows_service_config.txt" -Encoding utf8
...
if (-not $DryRun) {
    $firewallContent | Out-File -FilePath "config/firewall_config.bat" -Encoding utf8
