...
function New-BuildStructure {
    ...
    if (Test-Path -Path $buildDir) {
        ...
        Write-Host "  🗑️ Removed existing build directory" -ForegroundColor DarkGray
    }
...
function Invoke-UXTests {
    ...
    try {
        ...
        Write-Host "  ⚠️ Could not run UX tests: $($_.Exception.Message)" -ForegroundColor Yellow
...
function Invoke-FirebaseDeployment {
    ...
    try {
        ...
        $projectId = switch ($Environment) {
            ...
            "production" { "prometheus-trading" }
            default { "prometheus-trading" }
        }
...
function Test-DeploymentHealth {
    ...
    $baseUrl = switch ($Environment) {
        "staging" { "https://prometheus-staging.web.app" }
        "production" { "https://prometheus-trading.web.app" }
        default { "https://prometheus-trading.web.app" }
    }
    ...
    foreach ($url in $testUrls) {
        try {
            ...
            if ($response.StatusCode -eq 200) {
                ...
                Write-Host "    ⚠️ $url (${$response.StatusCode})" -ForegroundColor Yellow
                $allHealthy = $false
            }
        }
        catch {
...
try {
    ...
    try {
        ...
        Write-Host "  ⚠️ Please login to Firebase: firebase login" -ForegroundColor Yellow
    }
